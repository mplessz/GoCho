...
[ ] traiter un premier lot de vardep (les logits par ex, plus facile)
...
f_cem <- function(df) {
	MatchIt::matchit(formula = form_cem, data = df, method = "cem")
	}
...
df_qlte_cem <- cem_nest %>%
	...
	select(-data, -matched, -data_cem) %>% 
	 unnest(qualite_cem)

filter(df_qlte_cem, Participants =="Unmatched")
```
...
select(vardep, ends_with("p.value"))
